...
- **Hybrid Execution Mode**: The system supports two execution paths for LLM tasks:
  - **API Path** (Default): Uses direct calls to OpenAI/Anthropic.
  - **CLI Path**: Wraps local agent tools like `claude-code` via subprocess calls, allowing the system to leverage the full power of CLI-based autonomous agents.
